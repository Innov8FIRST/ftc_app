...
@Autonomous(name = "A_Innov8_Relic_RedStraight", group = "Auto")

public class A_Innov8_Relic_RedStraight extends LinearOpMode {
            ...
            switch (taskNumber) {
                case 0: // Drop jewel arm and check for color
                    if (teamColor == 1) {   // If blue team, then drop left arm to check for color
                        ...
                        robot.jewelArm.setPosition(0.3);
                ...
                case 4: // determine jewel color and set case; forward or backward
                    preNumber  =4;
                       ...
                       if (teamColor == 0) {  // If team red, then make decision based on color sensor
                           if (robot.OrangeySensor.blue() >= 50 && robot.OrangeySensor.red() <= 87) {
                                    taskNumber = 3;
                ...
                case 7: // Stopping in safe zone and drop glyph
                    telemetry.addData("current", taskNumber);
                    telemetry.update();

                    startPositionR = robot.rightMotor.getCurrentPosition();
                    endPositionR = startPositionR - 650;
                ...
                case 8: // moves forward
                    Time = 0;
                    telemetry.addData("current", taskNumber);
                    telemetry.update();

                    startPositionL = robot.leftMotor.getCurrentPosition();
                    endPositionL = startPositionL - 300;
                ...
                case 9:// turning right to cryptobox
                    startPositionR = robot.rightMotor.getCurrentPosition();
                    endPositionR = startPositionR + 650;
                    while (opModeIsActive() && robot.rightMotor.getCurrentPosition() <= endPositionR) {
                ...
                case 10:  //moving forward into cryptobox
                    startPositionR = robot.rightMotor.getCurrentPosition();
                ...
                case 11: //moving backwards
                    startPositionR = robot.rightMotor.getCurrentPosition();
                    endPositionR = startPositionR + 150;
                    while (opModeIsActive() && robot.rightMotor.getCurrentPosition() < endPositionR) {
                        robot.rightMotor.setPower(20);
                        robot.leftMotor.setPower(20);
                    }
